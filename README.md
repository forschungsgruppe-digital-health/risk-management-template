...
| [`docs/PMS.md`](docs/PMS.md) + [`pms-review.yml`](.github/workflows/pms-review.yml) | Post-market surveillance plan (ISO 14971 §10 / MDR Art. 83–92) wiring the §10 feeds (field-feedback, SBOM/CVE, watch rows) + a quarterly review action (opt-in `PMS_REVIEW_ENABLED`) that opens a §10.2–§10.4 checklist issue |
...
| [`scripts/soup-sbom-diff.sh`](scripts/soup-sbom-diff.sh) + [`check-harm-risk.sh`](scripts/check-harm-risk.sh) + [`risk-lint.yml`](.github/workflows/risk-lint.yml) | **Advisory drift lint** (warn-only): SOUP↔SBOM reconciliation (IEC 62304 §8.1.2) + harm-risk register completeness/consistency (§7.2/§7.3/§7.5/§8) → job summary on every PR. Operationalises the `mdr-audit-readiness` per-issue rules; never blocks (enable, not enforce) |
